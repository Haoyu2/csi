#!/usr/bin/env python
"""
Python reimplementation of the Widar 3.0 preprocessing pipeline with BAP support:
    CSI -> Doppler spectrum -> Body-coordinate Velocity Profile (BVP) + Body Acceleration Profile (BAP).
"""
...
class MappingConfig:
    ...
    wave_length: float = 299_792_458 / 5.825e9
...
def load_csi(path: pathlib.Path) -> np.ndarray:
    try:
        ...
        raise RuntimeError("csiread is required") from exc
...
def compute_doppler_spectrum(
    ...
    for rx in range(1, rx_cnt + 1):
        ...
        principal = pca.fit_transform(np.real(conj_mult)).squeeze(axis=-1)
...
def doppler_to_bvp_bap(
    ...
    acceleration_spectrum = np.zeros((M, M, seg_number))
    prev_doppler_tgt = None

    for ii in tqdm(range(seg_number), desc="Mapping Doppler -> BVP & BAP"):
        doppler_seg = doppler_spectrum[:, :, ii * seg_length : (ii + 1) * seg_length]
        doppler_tgt = doppler_seg.mean(axis=2)
        ...
        x0 = np.zeros(M * M)
        bounds_bvp = optimize.Bounds(lb=np.zeros_like(x0), ub=U_bound.flatten())
        res_bvp = optimize.minimize(
            dvm_loss,
            x0,
        ...
        velocity_spectrum[:, :, ii] = res_bvp.x.reshape(M, M)

        if prev_doppler_tgt is not None:
            doppler_diff = doppler_tgt - prev_doppler_tgt
            bounds_bap = optimize.Bounds(lb=-U_bound.flatten(), ub=U_bound.flatten())
            res_bap = optimize.minimize(
                dvm_loss,
                x0,
                args=(VDM, doppler_diff, map_cfg.lambda_, map_cfg.norm_l1),
                method="SLSQP",
                bounds=bounds_bap,
                options={"maxiter": map_cfg.MaxFunctionEvaluations, "ftol": 1e-6, "disp": False},
            )
            acceleration_spectrum[:, :, ii] = res_bap.x.reshape(M, M)
        else:
            acceleration_spectrum[:, :, ii] = 0.0

        prev_doppler_tgt = doppler_tgt.copy()

    orientation_deg = map_cfg.torso_ori[ori_sel - 1]

    def rotate_spectrum(spectrum):
        if M % 2 == 0:
            spectrum = np.pad(spectrum, ((1, 0), (1, 0), (0, 0)), mode="constant")
            rotated = np.stack(
                [
                    rotate(
                        spectrum[:, :, k],
    ...
    rotated_bvp = rotate_spectrum(velocity_spectrum)
    rotated_bap = rotate_spectrum(acceleration_spectrum)
    return rotated_bvp, rotated_bap


def parse_args():
    parser = argparse.ArgumentParser(description="End-to-end CSI -> Doppler -> BVP + BAP pipeline.")
    parser.add_argument("--base-prefix", required=True, help="Path prefix to clip")
    ...
    parser.add_argument("--out-dir", default="python_pipeline/output", help="Output directory")
    parser.add_argument("--save-mat", action="store_true", help="Also save .mat files")
...
def main():
    ...
    bvp, bap = doppler_to_bvp_bap(doppler, freq_bin, args.rx_cnt, args.pos, args.ori)
    out_path = out_dir / f"{args.people}-{args.motion}-{args.pos}-{args.ori}-{args.ges}_bvp_bap.npz"
    np.savez_compressed(out_path, velocity_spectrum_ro=bvp, acceleration_spectrum_ro=bap)
    logging.info("Saved BVP and BAP -> %s", out_path)

    if args.save_mat:
        mat_path = out_path.with_suffix(".mat")
        sio.savemat(mat_path, {"velocity_spectrum_ro": bvp, "acceleration_spectrum_ro": bap})
        logging.info("Saved Matlab-compatible output -> %s", mat_path)
